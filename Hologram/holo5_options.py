...
def start():
    ...
    while(True):
        ...
        #rec 1
        if in_rectangle(width-50,0,width,50,gcx,gcy) or current==1:
            current=1
            flag2, framevid = video.read()
        ...
        #rec 2
        if in_rectangle(width-50,60,width,110,gcx,gcy) or current==2:
            current=2


        #rec 3 - Paint
        if in_rectangle(width-50,120,width,170,gcx,gcy) or current==3:
            ...
            cv2.rectangle(img_cpy,(width-50,00),(width,50),(0,0,0),-1) #BlackI
            cv2.putText(img_cpy,"1", (width-37,40), cv2.FONT_HERSHEY_SIMPLEX, 1.4,(255,255,255), 3)
            cv2.rectangle(img_cpy,(width-50,60),(width,110),(0,0,0),-1) #BlackII
            cv2.putText(img_cpy,"2", (width-37,100), cv2.FONT_HERSHEY_SIMPLEX, 1.4,(255,255,255), 3)
            cv2.rectangle(img_cpy,(width-50,120),(width,170),(0,0,0),-1) #BlackIII
            cv2.putText(img_cpy,"3", (width-37,160), cv2.FONT_HERSHEY_SIMPLEX, 1.4,(255,255,255), 3)
